...
/// Set expect for fostlib::ua
fg::frame::builtin fg::lib::expect = [](fg::frame &stack,
                                        fg::json::const_iterator pos,
                                        fg::json::const_iterator end) {
    auto http_method =
            stack.resolve_string(stack.argument("http_method", pos, end));
    auto url =
            fostlib::url{stack.resolve_string(stack.argument("url", pos, end))};
    auto response_status =
            stack.resolve_int(stack.argument("response_status", pos, end));
    if (response_status >= 400) {
        switch (response_status) {
        case 401:
            fostlib::ua::expect(
                    http_method, url, fostlib::ua::unauthorized{url});
            break;
        case 403:
        ...
        case 410:
            fostlib::ua::expect(
                    http_method, url, fostlib::ua::resource_not_found{url});
            break;
        default:
            fostlib::ua::expect(
                    http_method, url,
                    fostlib::ua::http_error{url,
        ...
        throw fostlib::exceptions::not_implemented{
                __PRETTY_FUNCTION__,
                "HTTP Status " + std::to_string(response_status)
                        + " not implemented"};
    }
    return fostlib::json();
};
