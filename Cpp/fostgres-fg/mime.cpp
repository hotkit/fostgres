...
namespace {
    ...
    fg::json mime_to_json(const fostlib::mime &body) {
        ...
        return fostlib::json::parse(fostlib::coerce<fostlib::string>(
...
std::unique_ptr<fostlib::binary_body>
        fg::mime_from_argument(frame &stack, const json &data) {
    fostlib::json expr;
    try {
        std::unique_ptr<fostlib::binary_body> body;
        expr = stack.resolve(data);
        if (const auto fn =
                    fostlib::coerce<fostlib::nullable<f5::u8view>>(expr);
            fn) {
...
void fg::assert_comparable(
        const fostlib::mime &actual, const fostlib::mime &expected) {
    //     if ( actual.headers()["Content-Type"].value() !=
    //     expected.headers()["Content-Type"].value() ) {
    //         fostlib::exceptions::not_implemented error(__func__,
    //         "Content-Type mismatch"); fostlib::insert(error.data(), "actual",
    //         actual.headers()["Content-Type"].value());
    //         fostlib::insert(error.data(), "expected",
    //         expected.headers()["Content-Type"].value()); throw error;
    //     }
    if (actual.headers()["Content-Type"].value() == "application/json") {
        ...
        for (auto row : expected_body["rows"]) {
            ...
            for (std::size_t index{}; index < expected_body["columns"].size();
                 ++index) {
                fostlib::insert(
                        expected_row, expected_body["columns"][index],
                        row[index]);
            }
            if (actual_iter == actual_end) {
                throw fostlib::exceptions::not_implemented(
                        __func__, "Found extra row in the expected data",
                        expected_row);
            }
            ...
            if (contains) {
                ...
                fostlib::insert(
                        error.data(), "mismatch", "path", contains.value());
                fostlib::insert(
                        error.data(), "mismatch", "expected",
                        expected_row[contains.value()]);
                fostlib::insert(
                        error.data(), "mismatch", "actual",
                        actual_json[contains.value()]);
                throw error;
            }
            ++actual_iter;
        }
        if (actual_iter != actual_end) {
            throw fostlib::exceptions::not_implemented(
                    __func__, "Found extra row in the actual data",
                    actual_iter.as_json());
        }
    } else {
        throw fostlib::exceptions::not_implemented(
