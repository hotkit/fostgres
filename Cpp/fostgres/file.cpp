...
fostlib::nullable<fostlib::json> fostgres::file_upload(
    ...
    if ( defn["source"].isnull() && row.has_key(name) ) {
        ...
        hasher << data;
        auto pathname = fostlib::coerce<fostlib::hex_string>(hasher.digest()).underlying().underlying();
        ...
        auto store_config = fostlib::setting<fostlib::json>::value(
            "File storage", fostlib::coerce<fostlib::string>(defn["store"]));
        auto location = fostlib::coerce<boost::filesystem::path>(store_config["path"]);
        location /= directory;
